...
class Engine(Service):
    ...
    def add_custom_chart(
        ...
        if display_on_price_chart:
            chart_obj["display_on_price_chart"] = display_on_price_chart
        if log_scale:
            chart_obj["log_scale"] = log_scale
        if color:
    ...
    def _configure(self):
        ...
        self._broker.register(
            "historical_sending_locked", self.__historical_sending_locked_event
        )
    ...
    async def __send_last_feed(self, service: SERVICES):
        last_feed = {"last_feed": [v[-1] for v in self.__data_buffer]}
    ...
    async def __send_debug_breakpoint(self):
        ...
        await self._broker.send(
            SERVICES.python_executor,
            "debug_breakpoint",
            DebugBreakpoint(**breakpoint_params),
        )
    ...
    async def __historical_sending_locked_event(self):
        await self._broker.send(
            SERVICES.historical_data_feeds, "unlock_historical_sending"
        )
    ...
    async def __data_finish_event(self):
        ...
        await self._broker.send(
            SERVICES.python_backtester, "data_finish", DataFinish(**finish_params)
        )
    ...
    async def __get_buffer_length_event(self, service_name):
        await self._broker.send(
            SERVICES[service_name], "engine_set_buffer_length", self.__buffer_length
        )
    ...
    async def __register_price_event(self, price_event):
        ...
        symbol: DataSymbol = [
            data
            for data in self.__data_schema.data
            if data.identifier == price_event.symbol_itentifier
        ][0]
